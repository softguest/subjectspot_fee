...
import { students, users, parentsStudents, payments, classFeeInstallments, studentFees } from "@/config/schema";
...
export async function POST(req: Request) {
  try {
    ...
    const { studentId, installmentId, studentFeeId, amount, momoTransactionId } = body;

    // Verify student belongs to parent
    const link = await db.query.parentsStudents.findFirst({
    where: and(
        eq(parentsStudents.parentUserId, userId),
        eq(parentsStudents.studentId, studentId)
    ),
    });

    if (!link) return NextResponse.json({ error: "Student not linked to parent" }, { status: 403 });

    // Optional: validate installment or studentFee exists
    if (installmentId) {
